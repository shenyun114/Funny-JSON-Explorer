class DataVisualization:
    ...
    def getConversion(self, data, level=0, parent_last=[]):
        pass
...
class TreeVisualization(DataVisualization):
    ...
    def getRes(self):
        self.getConversion(self.data)
        self.rebuildOutput()
        return self.output

    def getConversion(self, data=None, level=0, parent_last=[]):
        if data is None:
            data = self.data
        # 可视化为树形
        for i, (key, value) in enumerate(data.items()):
            ...
            if isinstance(value, dict):
                ...
                self.getConversion(value, level + 2, parent_last + [is_last])
            else:
